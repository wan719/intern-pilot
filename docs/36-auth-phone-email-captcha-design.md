...
## 二十五、Trae + DeepSeek Pro 执行提示词

```text
你现在是我的 Java 全栈项目开发助手，请基于 docs/36-auth-phone-email-captcha-design.md 实现 InternPilot 的手机号/邮箱注册登录与验证码机制。

项目背景：
InternPilot 是 Spring Boot + Spring Security + JWT + MyBatis-Plus + MySQL + Redis + Vue3 的 AI 实习投递平台。
当前项目已经有 JWT 登录、RBAC 权限、admin/demo 默认账号、Docker 部署和 Redis。
当前登录方式是 username + password。
现在要改为手机号或邮箱注册登录，并增加验证码机制。

执行要求：
1. 先检查当前分支名称，必须是 feature/auth-phone-email-captcha。
2. 不要自动提交 Git。
3. 不要修改和认证无关的大量业务逻辑。
4. 不要删除 RBAC。
5. 不要删除 MockAiClient。
6. 不要泄露 API Key。
7. 不要真实发送短信，默认使用 MockCaptchaSender。
8. 不要破坏 admin/demo 演示账号登录。
9. 保留 JWT 鉴权和原有权限体系。

任务一：数据库字段增强
1. 用户表增加 phone、email、account_type、phone_verified、email_verified、last_login_time 字段。
2. User 实体同步字段。
3. init.sql、dev-init-rbac.sql、test-schema.sql、test-data.sql 同步。
4. admin/demo 默认账号补充 phone/email。
5. 不删除 username 字段。

任务二：验证码模块
1. 新增 AccountTypeEnum：PHONE、EMAIL、USERNAME、SYSTEM。
2. 新增 CaptchaSceneEnum：PHONE_REGISTER、EMAIL_REGISTER。
3. 新增 CaptchaService。
4. 新增 CaptchaSender。
5. 新增 MockCaptchaSender。
6. Redis 存储验证码，TTL 5 分钟。
7. 发送冷却 60 秒。
8. 错误次数限制 5 次。
9. Mock 模式验证码固定为 123456，方便演示和测试。
...
4. 登录时支持 email 或 phone。
5. 普通用户不再使用 username 登录。
6. admin/demo 系统账号可以兼容 username 登录，避免演示账号失效。
7. 登录成功后继续返回 JWT 和用户权限。

任务四：前端改造
1. 登录页“用户名”改为“手机号 / 邮箱”。
2. 登录请求参数改为 account + password。
3. 注册页增加邮箱/手机号选择。
4. 注册页增加验证码输入框。
5. 注册页增加发送验证码按钮和 60 秒倒计时。
6. auth.ts 增加 sendRegisterCaptchaApi。
7. 默认演示账号提示更新为 demo@internpilot.local / 123456 和 admin@internpilot.local / 123456。

任务五：配置
1. application.yml 增加 auth.captcha 配置。
2. application-dev.yml 和 application-test.yml 默认 mock。
3. application-prod.yml 支持 AUTH_CAPTCHA_MODE。
4. deploy/.env.example 增加 AUTH_CAPTCHA_MODE=mock。
...
```text
你现在是我的 Java 全栈项目代码复查与收尾助手，请检查 Trae + DeepSeek Pro 对手机号/邮箱注册登录与验证码机制的实现结果。

请严格参考 docs/36-auth-phone-email-captcha-design.md。

重点复查：
1. 登录页是否不再使用用户名作为普通登录方式。
2. 是否支持手机号登录。
3. 是否支持邮箱登录。
4. 是否支持手机号注册。
5. 是否支持邮箱注册。
...
10. 验证码是否有错误次数限制。
11. Mock 验证码是否适合本地演示。
12. admin/demo 是否仍可登录。
13. JWT 和 RBAC 是否未被破坏。
14. 前端 auth.ts 是否同步改造。
15. 登录注册页面是否能正常 build。
16. init.sql、test-schema.sql、test-data.sql 是否同步用户字段。
17. Docker .env.example 是否有 AUTH_CAPTCHA_MODE。
18. 是否泄露 API Key。
19. 是否真实调用短信服务。
20. 是否有测试覆盖核心场景。
