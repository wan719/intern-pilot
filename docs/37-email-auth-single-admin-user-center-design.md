...
## 十六、Trae + DeepSeek Pro 执行提示词

```text
你现在是我的 Java 全栈项目开发助手，请在 InternPilot 项目中实现“邮箱注册登录、单管理员初始化、用户中心页面”。

当前分支：
feature/auth-phone-email-captcha

请严格参考：
docs/37-email-auth-single-admin-user-center-design.md

当前要求：
1. 第一阶段只做邮箱验证码，不做手机验证码。
2. 使用 QQ 邮箱 SMTP 作为真实邮箱验证码发送方式。
3. 删除 demo 演示账号初始化。
4. 删除旧 admin / 123456 演示账号初始化。
5. init.sql 中只初始化一个管理员账号：3425446714@qq.com。
6. 不公开该管理员密码。
7. 新增用户中心页面。
8. 线上系统使用真实邮箱验证码。
9. 测试环境继续使用 MockCaptchaSender。
10. 不要真实发送邮件，除非我明确要求。
11. 不要自动提交 Git。
12. 不要 push。
...
1. 修改 init.sql。
2. 删除 demo 用户初始化。
3. 删除旧 admin / 123456 用户初始化。
4. 只初始化 3425446714@qq.com 管理员。
5. 给该用户绑定 ADMIN 角色。
6. 密码使用 BCrypt 哈希，不要写明文。
7. 同步 dev-init-rbac.sql、test-schema.sql、test-data.sql。
8. 如果需要，新增 prod-single-admin-reset.sql，用于清理服务器已有 demo/admin 数据。
9. 不破坏 RBAC 权限数据。
...
2. 实现 EmailCaptchaSender。
3. 支持 QQ 邮箱 SMTP。
4. 读取 MAIL_HOST、MAIL_PORT、MAIL_USERNAME、MAIL_PASSWORD、MAIL_FROM、MAIL_SSL_ENABLED、MAIL_STARTTLS_ENABLED。
5. 邮件标题：InternPilot 注册验证码。
6. 邮件正文中文。
7. SMTP 配置缺失时返回清晰错误。
8. 测试环境使用 MockCaptchaSender。
9. 手机验证码 provider 设置为 disabled。
...
3. 登录页只显示邮箱。
4. 登录接口使用邮箱 + 密码。
5. 普通用户不再使用 username 登录。
6. 3425446714@qq.com 管理员可以登录。
7. JWT 和 RBAC 不受影响。

任务四：用户中心
1. 新增 GET /api/user/profile。
2. 新增 PUT /api/user/profile。
3. 新增 PUT /api/user/password。
4. 只能查看和修改当前登录用户。
5. 新增前端 UserCenter.vue。
6. 右上角用户菜单增加“个人中心”入口。
7. 用户中心展示邮箱、角色、邮箱验证状态、最近登录时间。
8. 支持修改昵称。
9. 支持修改密码。

任务五：README 和配置
1. deploy/.env.example 增加 QQ 邮箱 SMTP 配置项。
2. README 删除 demo/admin 公开账号说明。
3. README 说明线上管理员账号不公开。
4. README 增加邮箱验证码配置说明。
5. 不要写入真实 MAIL_PASSWORD。
6. 不要泄露 API Key。
...
3. 修改了哪些文件
4. init.sql 如何变化
5. 是否删除 demo 和旧 admin 初始化
6. 3425446714@qq.com 管理员如何初始化
7. 邮箱验证码如何实现
8. QQ 邮箱 SMTP 如何配置
9. 用户中心新增了哪些接口和页面
10. 是否影响 JWT/RBAC
...
```text
你现在是我的 Java 全栈项目代码复查与收尾助手，请检查“邮箱注册登录、单管理员初始化、用户中心页面”的实现结果。
...
2. 手机号注册是否隐藏或禁用。
3. 是否使用邮箱验证码注册。
4. 是否实现 EmailCaptchaSender。
5. QQ 邮箱 SMTP 配置是否通过环境变量读取。
6. 是否没有泄露 MAIL_PASSWORD。
7. 测试环境是否使用 MockCaptchaSender。
8. init.sql 是否不再初始化 demo。
9. init.sql 是否不再初始化旧 admin / 123456。
10. init.sql 是否只初始化 3425446714@qq.com 管理员。
11. 该用户是否绑定 ADMIN 角色。
12. README 是否不公开管理员密码。
13. 是否新增用户中心页面。
14. 是否新增 GET /api/user/profile。
15. 是否新增 PUT /api/user/profile。
16. 是否新增 PUT /api/user/password。
17. 用户中心是否只能操作当前用户。
18. 修改密码是否校验旧密码。
19. JWT 是否正常。
20. RBAC 是否正常。
21. Docker .env.example 是否更新。
22. 后端测试是否通过。
23. 前端构建是否通过。
...
5. 是否确认邮箱注册可用
6. 是否确认邮箱登录可用
7. 是否确认 demo / 旧 admin 初始化已移除
8. 是否确认 3425446714@qq.com 是唯一初始化管理员
9. 是否确认用户中心可用
10. 是否确认 JWT/RBAC 未破坏
11. 后端测试结果
12. 前端构建结果
13. 是否建议提交 Git
